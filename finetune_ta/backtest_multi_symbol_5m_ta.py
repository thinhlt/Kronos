"""
Walk-forward backtest for the finetuned `multi_symbol_5m_ta` experiment.

Loads tokenizer + basemodel from finetuned/multi_symbol_5m_ta/ (via the
training config + local fallback), runs validation-split forecasts on local
data/*_kline_5min.csv, and writes results under backtests/multi_symbol_5m_ta/.
...
Usage:
    ...
    python backtest_multi_symbol_5m_ta.py --max-windows 2
    python backtest_multi_symbol_5m_ta.py --pred-len 288 --input data/BTCUSDT_kline_5min.csv
"""
...
if __name__ == "__main__":
    main_for_config(
        "configs/config_multi_symbol_5m_ta.yaml",
