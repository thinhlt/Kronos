...
from model_factory import build_initial_tokenizer, load_finetuned_tokenizer, build_predictor
...
class SequentialTrainer:
    ...
    def train_tokenizer_phase(self):
        ...
        if self.rank == 0:
            print(f"Initializing tokenizer (pre_trained={getattr(self.config, 'pre_trained_tokenizer', False)}, d_in={self.config.d_in})")
        tokenizer = build_initial_tokenizer(self.config)
        ...
        logger.info(f"Data files ({len(self.config.data_paths)}): {self.config.data_paths}")
    ...
    def train_basemodel_phase(self):
        ...
        # Always load the tokenizer that phase 1 actually trained and saved --
        # regardless of whether it started from pretrained weights or from
        # scratch, the predictor must be trained against the *finetuned*
        # tokenizer's tokens, not a freshly re-randomized one.
        if self.rank == 0:
            print("Loading finetuned tokenizer from phase 1...")
        tokenizer = load_finetuned_tokenizer(self.config)
        tokenizer = tokenizer.to(self.device)
        ...
        if getattr(self.config, 'pre_trained_predictor', True):
            ...
            logger.info("pre_trained_predictor=False, randomly initializing Predictor architecture")
            if self.rank == 0:
    ...
    def run_training(self):
        if self.rank == 0:
            print("Starting Kronos technical-indicator model sequential fine-tuning training")
...
def main():
    parser = argparse.ArgumentParser(description='Kronos Technical-Indicator Model Sequential Fine-tuning Training')
